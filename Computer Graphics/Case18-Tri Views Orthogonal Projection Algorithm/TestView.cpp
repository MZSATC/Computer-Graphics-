...
void CTestView::DoubleBuffer(CDC* pDC)//双缓冲
{
	...
	DrawWView(&memDC[2]);  //绘制侧视图
	DrawHView(&memDC[3]);  //绘制俯视图
	for(i=0;i<4;i++)
	{
		pViewDC[i]->BitBlt(rect.left,rect.top,rect.Width(),rect.Height(),&memDC[i],-rect.Width()/2,-rect.Height()/2,SRCCOPY);//将内存memDC中的位图拷贝到显示pDC中
